...
```protobuf
option go_package = "github.com/org/spatial-server/gen/proto/spatialserver/v1;spatialserverv1";
```

| Component | Rule |
|-----------|------|
| Module path | Matches Go module path from `go.mod` |
| Directory | `gen/proto/spatialserver/v1` |
| Package suffix | `spatialserverv1` (lowercase, version suffix) |

**Rules:**

- Generated Go code goes in `gen/proto/` (gitignored, generated at build time).
...
- Internal imports without path prefix (`import "common.proto"`).
...
| Check into VCS | No (gitignored, generated by `make gen/proto`) |
